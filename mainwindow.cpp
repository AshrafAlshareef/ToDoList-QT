...
void MainWindow::cellSelected(int nRow, int nCol)
   ...
   dlg.setModal(true);
   dlg.setData(
            table->item(nRow, 1)->text().toUtf8().constData(),
            table->item(nRow, 2)->text().toUtf8().constData(),
            table->item(nRow, 3)->text().toUtf8().constData(),
            table->item(nRow, 4)->text().toUtf8().constData()
            );
   dlg.setPath(path);
   dlg.setOrigin(this);
   dlg.exec();
...
void MainWindow::filter()
   ...
   for(i=0; i<data.size(); i++){
      ...
      if (rb_today->isChecked())
         ...
         {
         if(cur.tm_year+1900 < year ||
               (cur.tm_year+1900 == year && cur.tm_mon+1 < month) ||
               (cur.tm_year+1900 == year && cur.tm_mon+1 == month && cur.tm_mday <= day)
               )
            {
            data.erase(data.begin()+i);
            i--;
